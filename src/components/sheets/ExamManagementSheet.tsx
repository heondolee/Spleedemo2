import { Plus, ChevronDown, ChevronRight, Calendar, CheckSquare, Square, Trash2, ZoomIn, ZoomOut, Settings, GripVertical } from 'lucide-react';
...
export function ExamManagementSheet() {
      ...
      subjects: [
        {
          ...
          name: '수학',
          ranges: [
            { id: 'r1', text: '1단원: 다항식', completed: true },
            { id: 'r2', text: '2단원: 방정식', completed: false, deadline: '2026-03-10' },
          ],
        ...
        },
        {
          ...
          name: '영어',
          ranges: [
            { id: 'r3', text: '1-3과', completed: false },
          ],
          strategy: '',
          memo: '',
          progress: 20,
        ...
        ? {
            ...
            subjects: exam.subjects.map(subject =>
              subject.id === subjectId 
                ? {
                    ...subject,
                    ranges: subject.ranges.map(range =>
                      range.id === rangeId ? { ...range, text } : range
                    )
                  }
                : subject
            )
        ...
        ? {
            ...
            subjects: exam.subjects.map(subject => {
              if (subject.id === subjectId) {
                const updatedRanges = subject.ranges.map(range =>
                  range.id === rangeId ? { ...range, completed: !range.completed } : range
                );
                const completedCount = updatedRanges.filter(r => r.completed).length;
                const progress = updatedRanges.length > 0 ? Math.round((completedCount / updatedRanges.length) * 100) : 0;
  ...
  const updateRangeDeadline = (examId: string, subjectId: string, rangeId: string, deadline: string) => {
    setExams(exams.map(exam => 
      exam.id === examId 
        ? {
            ...exam,
            subjects: exam.subjects.map(subject =>
              subject.id === subjectId 
                ? {
                    ...subject,
                    ranges: subject.ranges.map(range =>
                      range.id === rangeId ? { ...range, deadline } : range
                    )
                  }
                : subject
            )
        ...
        ? {
            ...
            subjects: exam.subjects.map(subject =>
              subject.id === subjectId 
                ? {
                    ...subject,
                    ranges: subject.ranges.filter(r => r.id !== rangeId)
                  }
                : subject
            )
          }
  ...
  const zoomOut = () => {
    setZoomLevel(Math.max(70, zoomLevel - 10));
  };
      ...
      <div className="flex items-center justify-between px-[16px] border-b border-border" style={{ height: '56px' }}>
        ...
        <div className="flex items-center gap-[12px]">
          <button
            ...
            className={`px-[12px] py-[6px] rounded-[8px] border transition-colors font-medium flex items-center gap-[6px] ${
              showDeadlines ? 'bg-primary text-primary-foreground border-primary' : 'border-border hover:bg-accent'
            }`}
            style={{ fontSize: '13px' }}
          >
          ...
          <div className="flex items-center gap-[8px] px-[12px] py-[6px] rounded-[8px] border border-border">
            <button
              onClick={zoomOut}
              className="w-[24px] h-[24px] flex items-center justify-center rounded-[6px] hover:bg-accent transition-colors"
              disabled={zoomLevel <= 70}
            >
              <ZoomOut className="w-[14px] h-[14px]" />
            </button>
            <span className="text-muted-foreground font-medium" style={{ fontSize: '13px', width: '50px', textAlign: 'center' }}>
              {zoomLevel}%
            </span>
            <button
              onClick={zoomIn}
                                          ...
                                          {subject.ranges.map((range) => (
                                            <div
                                              ...
                                              className={`flex items-center gap-[8px] px-[10px] rounded-[8px] border border-border bg-background transition-all ${
                                                editingItemId === range.id ? 'py-[12px]' : 'py-[8px]'
                                              }`}
                                            >
                                              {/* Checkbox */}
                                              <button
                                                onClick={() => toggleRangeComplete(exam.id, subject.id, range.id)}
                                              ...
                                              <input
                                                type="text"
                                                value={range.text}
                                                onChange={(e) => updateRange(exam.id, subject.id, range.id, e.target.value)}
                                                onFocus={() => setEditingItemId(range.id)}
                                                onBlur={() => setEditingItemId(null)}
                                                placeholder="범위를 입력하세요"
                                                className={`flex-1 bg-transparent outline-none transition-all ${
                                                  range.completed ? 'line-through text-muted-foreground' : ''
                                                }`}
                                                style={{ fontSize: '13px' }}
                                              />

                                              {/* Deadline (when enabled and editing) */}
                                              {showDeadlines && editingItemId === range.id && (
                                                <div className="flex items-center gap-[6px]">
                                                  <Calendar className="w-[14px] h-[14px] text-muted-foreground" />
                                                  <input
                                                    type="date"
                                                    value={range.deadline || ''}
                                                    onChange={(e) => updateRangeDeadline(exam.id, subject.id, range.id, e.target.value)}
                                                    className="bg-transparent text-muted-foreground outline-none border-b border-border hover:border-primary focus:border-primary transition-colors"
                                                    style={{ fontSize: '11px' }}
                                                  />
                                                </div>
                                              )}

                                              {/* Deadline Display (when not editing) */}
                                              {showDeadlines && editingItemId !== range.id && range.deadline && (
                                                <div className="flex items-center gap-[4px] text-muted-foreground" style={{ fontSize: '11px' }}>
                                                  <Calendar className="w-[12px] h-[12px]" />
                                                  {new Date(range.deadline).toLocaleDateString('ko-KR', { month: 'short', day: 'numeric' })}
                                                </div>
                                              )}
                                              ...
                                              {editingItemId === range.id && (
                                                <button
                                                  onClick={() => deleteRange(exam.id, subject.id, range.id)}
